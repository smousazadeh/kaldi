...
class ArpaLmCompilerImpl : public ArpaLmCompilerImplInterface {
 ...
 private:
  StateId AddStateWithBackoff(HistKey key, Weight backoff);
  void CreateBackoff(HistKey key, StateId state, Weight weight);
...
ArpaLmCompilerImpl<HistKey>::ArpaLmCompilerImpl(
  ...
  // If substituting epsilons and treating <s> and </s> as epsilons, the start
  // state is same as the 0-gram state; otherwise, the 0-gram state is
  // separate, and the arc from the start state is accepting the <s> symbol.
  StateId zerogram = start;
  if (sub_eps_ == 0) {
    zerogram = fst_->AddState();
    fst->AddArc(start, fst::StdArc(bos_symbol_, bos_symbol_, 0, zerogram));
  }

  // In any case, the zerogram is the parent for unigram models...
  history_[HistKey()] = zerogram;
  // ... and for any n-gram starting with <s>
  const Symbol* bosptr = &bos_symbol_;
  history_[HistKey(bosptr, bosptr + 1)] = zerogram;
  ...
  // optimization saves aboit 2% states in an average grammar.
  if (sub_eps_ == 0) {
...
void ArpaLmCompilerImpl<HistKey>::ConsumeNGram(
  ...
  Weight weight = -ngram.logprob;
  if (sym == eos_symbol_) {
    // In some models, </s> n-grams have both prob and bow. This is not
    // probably correct, but given such a model, the most reasonable is to
    // sum the n-gram and bow logprobs.
    weight += -ngram.backoff;
    if (sub_eps_ == 0) {
      // Keep </s> as a real symbol not substituting.
      dest = eos_state_;
    } else {
...
inline void ArpaLmCompilerImpl<HistKey>::CreateBackoff(
    HistKey key, StateId state, Weight weight) {
  typename HistoryMap::iterator dest_it = history_.find(key);
...
void ArpaLmCompiler::ConsumeNGram(const NGram& ngram) {
  ...
  // Ignore the <s> unigram, as it is always implicitly added.
  if (ngram.words.size() == 1 && ngram.words[0] == Options().bos_symbol)
    return;
