...
/*
  ...
  the center frequency f of the bin:
     diameter = 40 + 80 f / (f + 900).
  so it increases from 40Hz to 120Hz with a knee around 900Hz.
  ...
  the next bin would be less than 0.5.  By this I mean is the y-value where the
  raised-cosines cross.  This value ensures that there won't be too a 'dip'
  in the middle of the two bins.  Now, cos(pi/3) = 0.5, so at the maximum
  allowed separation of bins, they would be separated by 2pi/3.  The diameter
  being pi, this means that the diameter cannot be less than 3/2 times the
  separation of bins.
 */
void MelBanks::ComputeModifiedBins() {
  ...
  for (int32 bin = 0; bin < num_bins; bin++) {
    ...
    BaseFloat diameter_floor = (next_center - center_freq) * 1.5,
        diameter = 40.0 + 80.0 * (center_freq / (center_freq + breakpoint_));
    if (diameter < diameter_floor)
      diameter = diameter_floor;
...
void MelBanks::SetConfigs(const MelBanksOptions &opts,
  ...
  breakpoint_ = (opts.modified ? 900.0 : 700.0);
