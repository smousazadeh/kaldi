...
# _4o is as _4m, but with --two-level-tree-scale 0.5 - an interpolation between
# using and not using the two-level-tree-scale.
# worse.
# ./compare_wer.sh 4m 4o
# System                       4m        4o
# WER on train_dev(tg)      16.75     17.14
# WER on train_dev(fg)      15.35     15.74
# WER on eval2000(tg)        18.4      18.4
# WER on eval2000(fg)        16.8      16.7
# Final train prob       -0.10169 -0.103696
# Final valid prob      -0.120332 -0.122786

# _4m is as _4l (also compare 4f), but with 0.00005 as the l2 regularization weight,
# not 0.0001.

# _4l is as _4f, but using the 2-level tree for modified application of l2 regularization;
# and increasing the constant again from 0.00005 to 0.0001 as I think a higher value will
# work better in this setup.  Using --leaves-per-group 10.
...
get_egs_stage=-10
speed_perturb=true
dir=exp/chain/tdnn_4o # Note: _sp will get added to this if $speed_perturb == true.
...
treedir=exp/chain/tri5_4l_tree$suffix
...
if [ $stage -le 11 ]; then
  ...
  steps/nnet3/chain/build_tree.sh --stage -5 --frame-subsampling-factor 3 \
      --leaves-per-group 10 \
...
if [ $stage -le 12 ]; then
 ...
 steps/nnet3/chain/train_tdnn.sh --stage $train_stage \
    --egs-dir exp/chain/tdnn_4l_sp/egs \
    --two-level-tree-scale 0.5 \
