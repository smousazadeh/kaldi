...
parser.add_argument("--utt2dur", type = string,
                    help="""Filename of input 'utt2dur' file (needed only if
                    --seconds-per-spk-max is provided)""")
parser.add_argument("--respect-speaker-info", type = string, default = 'true',
                    choices = ['true', 'false'],
...
while True:
    ...
    a = line.split()
    if len(a) != 2:
        sys.exit("modify_speaker_info.sh: bad utt2spk line from standard input (expected two fields): " +
                 line)
    [ utt, spk ] = a
    utt2spk[utt] = spk
...
if args.seconds_per_spk_max > 0:
    ...
    try:
        ...
        while True:
            ...
            a = line.split()
            if len(a) != 2:
                sys.exit("modify_speaker_info.sh: bad utt2dur line from standard input (expected two fields): " +
                         line)
            [ utt, dur ] = a
            utt2dur[utt] = float(dur)
        for utt in utt2spk:
            if not utt in utt2dur:
                sys.exit("modify_speaker_info.sh: utterance {0} not in utt2dur file {1}".format(
                        utt, args.utt2dur))
    except Exception as e:
        sys.exit("modify_speaker_info.sh: problem reading utt2dur info: " + str(e))
...
if args.respect_speaker_info == 'true':
    ...
    uttlists = SplitIntoGroups(sorted(keys(utt2spk)))
