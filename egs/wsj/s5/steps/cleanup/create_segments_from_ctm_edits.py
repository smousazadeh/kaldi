...
# Segment data using 'ctm edits' (i.e. the output of get_ctm_edits.py)
...
def GetArgs():
    parser = argparse.ArgumentParser(description =
        ...
        "(i.e. the output of get_ctm_edits.py")
    ...
    # Required arguments
    parser.add_argument("ctm_edits",
                        help = "Filename of input word-edit information in extended-CTM format, "
                        "as produced by steps/cleanup/get_ctm_edits.py")
    parser.add_argument("segments_out", metavar = "<segments-out>",
                        help = "Filename of output sub-segments (sub-segments is like a segments "
                        "file, but with utterance-id not recording-id in 2nd field; it's interpreted "
                        "by steps/cleanup/do_cleanup_segmentation.py).")
    parser.add_argument("text_out", metavar = "<text-out>",
                        help = "Output sub-segmented text")
...
class CtmEditsProcessor:
    def __init__(self, args):
        ...
        # ctm_edits is a dict from (string) utterance-id to
        # the ctm for that utterance, which is an array of 8-dimensional arrays,
        # each with the format:
        # [ file-id, channel, start-time, end-time, confidence, hyp-word, ref-word, edit ]
        # is an
        self.ctm_edits = dict()
    ...
    def LoadCtmEdits(self, ctm_edits_filename):
        # Load the ctm with edits appended into a dictionary
        ctm_edits = self.ctm_edits

        with open(ctm_edits_filename, 'r') as f:
            for l in f:
                splits = l.strip().split()
                # convert the begin-time and duration to integer frame indexes
                frame_shift_secs = self.config.frame_shift / 1000.0
                splits[2] = int(float(splits[2]) / frame_shift_secs + 0.0001)    # begin time
                splits[3] = int(float(splits[3]) / frame_shift + 0.5)      # duration
                ...
                utt = splits[0]
                ctm_edits.setdefault(utt, []).append(tuple(splits))

    # Counts the number of word errors for each token in the processed CTM
    # starting from seg_start to seg_end
    def GetRunningWordErrors(self, utt_ctm_edits, seg_start, seg_end):
        num_word_errors = 0
        num_words = 0
        running_word_errors = []
        for i in range(seg_start, seg_end):
            if (utt_ctm_edits[i][-1] in ['D','I','S']):
                num_word_errors += 1
            if (utt_ctm_edits[i][-2] != self.config.silence_symbol):
                num_words += 1
    ...
    def AddTextToSegments(self, utt, segments, frames_to_include):
        ...
        for n in range(0, len(segments)):
            ...
            while i > 0 and utt_ctm_edits[i][2] > beg:
                i -= 1
            ...
            while sum(utt_ctm_edits[i][2:4]) <= beg:
                i += 1
            ...
            if (utt_ctm_edits[i][2] < beg and
                    utt_ctm_edits[i][-1] not in ['SC','SD','SS','SI','I']):
                beg = max(0, min(utt_ctm_edits[i][2] - self.config.min_length_for_adding_unk, beg))
            ...
            #          beg                    end
            #                                     i
            while i < len(utt_ctm_edits) and sum(utt_ctm_edits[i][2:4]) < end:
                if utt_ctm_edits[i][-2] != self.config.silence_symbol:
                    text_for_seg += " " + utt_ctm_edits[i][-2]
                    num_nonsil_words += 1
                i += 1

            if (i < len(utt_ctm_edits) and utt_ctm_edits[i][2] < end and
                    utt_ctm_edits[i][-1] not in ['SC','SD','SS','SI','I']):
                end = min(sum(utt_ctm_edits[-1][2:4]), max(utt_ctm_edits[i][2] + self.config.min_length_for_adding_unk, end))
                self.stats.num_oov_symbols_added += 1
                text_for_seg += " " + self.config.oov_symbol
    ...
    def CreateSegments(self, utt_ctm_edits, utt, seg_start,
                       seg_end, frames_to_add_left, frames_to_add_right):
        # Compute start and end frames corresponding to the segment under
        # consideration
        start_frame = utt_ctm_edits[seg_start][2]
        end_frame = utt_ctm_edits[seg_end-1][2] + utt_ctm_edits[seg_end-1][3] + 1
        ...
        for i in range(seg_start, seg_end):
            if utt_ctm_edits[i][-1] in ['C','XS']:
                for x in range(utt_ctm_edits[i][2], sum(utt_ctm_edits[i][2:4]) + 1):
                    frames_to_include[x] = 1
            if utt_ctm_edits[i][-1] in kSilenceEdits:
                for x in range(utt_ctm_edits[i][2], sum(utt_ctm_edits[i][2:4]) + 1):
        ...
        running_word_errors = self.GetRunningWordErrors(utt_ctm_edits,
                                                        seg_start, seg_end)
        ...
        if (float(running_word_errors[-1][0]) /
            ...
            while i < seg_end:
                ...
                if (incorrect_start == -1
                    and utt_ctm_edits[i][-1] not in ['C','XS']):
                    incorrect_start = i

                # Find end of split point
                if (incorrect_start >= 0 and utt_ctm_edits[i][-1] in ['C','XS']):
                    if (running_word_errors[i-seg_start][0] -
                        ...
                        if incorrect_start > 0:     # Ignore the boundary
                            if utt_ctm_edits[incorrect_start][-1] == 'SC':
                                right_padding = min(self.config.silence_padding_correct, utt_ctm_edits[incorrect_start][3])
                            ...
                            elif utt_ctm_edits[incorrect_start][-1] in kSilenceEdits:
                                right_padding = min(self.config.silence_padding_incorrect, utt_ctm_edits[incorrect_start][3])
                                if right_padding > self.config.min_silence_padding:
                                    right_padding = random.randint(self.config.min_silence_padding, right_padding)
                            else:
                                right_padding = 0

                            for n in range(utt_ctm_edits[incorrect_start][2], utt_ctm_edits[incorrect_start][2] + right_padding):
                                frames_to_include[n] = 1

                        if i > 0:                   # Ignore the boundary
                            if utt_ctm_edits[i-1][-1] == 'SC':
                                left_padding = min(self.config.silence_padding_correct, utt_ctm_edits[i-1][3])
                                if left_padding > self.config.min_silence_padding:
                                    left_padding = random.randint(self.config.min_silence_padding, left_padding)
                            elif utt_ctm_edits[i-1][-1] in kSilenceEdits:
                                left_padding = min(self.config.silence_padding_incorrect, utt_ctm_edits[i-1][3])
                                if left_padding > self.config.min_silence_padding:
                                    left_padding = random.randint(self.config.min_silence_padding, left_padding)
                            else:
                                left_padding = 0

                            for n in range(utt_ctm_edits[i][2] - left_padding, utt_ctm_edits[i][2]):
                                frames_to_include[n] = 1
                        ...
                        # word errors
                        for n in range(utt_ctm_edits[incorrect_start][2],
                                       sum(utt_ctm_edits[i-1][2:4]) + 1):
            ...
            if incorrect_start >= 0:
                if (running_word_errors[-1][0] -
                    ...
                    # word errors
                    for n in range(utt_ctm_edits[incorrect_start][2],
                                   sum(utt_ctm_edits[seg_end-1][2:4]) + 1):
                        frames_to_include[n] = 3
                else:
                    self.stats.segments_ignored_on_max_incorrect_words += 1
    ...
    def SegmentUtterance(self, utt):
        ...
        while i < len(utt_ctm_edits):
            if (utt_ctm_edits[i][-1] == 'SC' and
                utt_ctm_edits[i][3] > self.config.max_silence_length):
                # Remove long silence regions that have correct words
                # on both sides.

                frames_to_add_right = min(self.config.silence_padding_correct, utt_ctm_edits[i][3])
                if frames_to_add_right > self.config.min_silence_padding:
                    random.randint(self.config.min_silence_padding, frames_to_add_right)

                if (seg_start < i):
                    num_segments += self.CreateSegments( utt_ctm_edits, utt,
                            seg_start, i,
                            frames_to_add_left, frames_to_add_right)
                    self.stats.segments_split_on_max_silence_length += 1

                frames_to_add_left = random.randint(self.config.min_silence_padding, min(self.config.silence_padding_correct, utt_ctm_edits[i][3]))
                if frames_to_add_left > self.config.min_silence_padding:
                    random.randint(self.config.min_silence_padding, frames_to_add_left)

                seg_start = i + 1
            i += 1
        if seg_start < len(utt_ctm_edits):
            num_segments += self.CreateSegments( utt_ctm_edits, utt, seg_start,
                                                 len(utt_ctm_edits),
                                                 frames_to_add_left, 0)

    def ProcessCtmEditsForUtt(self, utt):
        utt_ctm_edits = self.ctm_edits[utt]
        utt_ctm_edits.sort(key = lambda x:(x[2],x[2]+x[3])) # Sort by 'beg' time,
...
def Main():
    ...
    ctm_edits_processor = CtmEditsProcessor(args)
    ctm_edits_processor.LoadCtmEdits(args.ctm_edits)

    for utt in sorted(ctm_edits_processor.ctm_edits.keys()):
        ctm_edits_processor.ProcessCtmEditsForUtt(utt)

    ctm_edits_processor.PrintStats(sys.stderr)
