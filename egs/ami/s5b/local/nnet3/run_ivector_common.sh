...
if [ $stage -le 7 ]; then
  ...
  if [ -d exp/$mic/nnet3${cleanup_affix}/tri5/final.mdl ]; then
    # we don't want to overwrite old stuff, ask the user to delete it.
    echo "$0: exp/$mic/nnet3${cleanup_affix}/tri5/final.mdl already exists: please delete and then rerun."
    exit 1;
  fi
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 13 \
      ...
      $alidir exp/$mic/nnet3${cleanup_affix}/tri5
fi
...
if [ $stage -le 8 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
    ...
    data/$mic/${train_set}_sp_hires_subset 512 exp/$mic/nnet3${cleanup_affix}/tri5 exp/$mic/nnet3${cleanup_affix}/diag_ubm
fi
...
if [ $stage -le 9 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    data/$mic/${train_set}_sp_hires exp/$mic/nnet3${cleanup_affix}/diag_ubm exp/$mic/nnet3${cleanup_affix}/extractor || exit 1;
fi
...
if [ $stage -le 10 ]; then
  ...
  ivectordir=exp/$mic/nnet3${cleanup_affix}/ivectors_${train_set}_sp_hires_comb
  ...
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
    ...
    exp/$mic/nnet3${cleanup_affix}/extractor $ivectordir
  ...
  for data in dev eval; do
    ...
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj "$nj" \
      data/${mic}/${data}_hires exp/$mic/nnet3${cleanup_affix}/extractor \
      exp/$mic/nnet3${cleanup_affix}/ivectors_${data}_hires
  done
fi
